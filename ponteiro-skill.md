...
- **Commitar o backlog ao terminar um artigo**, sempre e sem perguntar: `python scripts/commit_listas.py --slug <slug>`. Desde 21/09/2026 a operacao e de maquina unica e o repo no GitHub esta arquivado, entao o commit e local: ele mantem o `BACKLOG-EDITORIAL.md` em dia para a pauta de amanha nao repetir a de hoje. No trigger diario isso ja vem embutido no `lock_diario.py --finish --status done`.
